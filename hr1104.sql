...
-- DEPARTMENTS 의 LOCATION_ID 와 LOCATIONS의 LOCATION_ID 조인)
...
-- 위치 ID가 1700인 동일한 사원들의 EMPLOYEE_ID, LAST_NAME, DEPARTMENT_ID, SALARY
-- 조회(EMPLOYEES 와 DEPARTMENTS 조인)
...
SELECT EMPLOYEE_ID, e1.first_name || ' ' || e1.last_name as name
FROM EMPLOYEES E1, EMPLOYEES E2
...
-- LAST_NAME에 u가 포함되는 사원들과 동일한 부서에 근무하는 사원들의 employee_id, last_ name
-- 조회
SELECT employee_id, LAST_NAME
FROM EMPLOYEES
WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID FROM LAST_NAME LIKE '%u%');
...
SELECT MANAGER_ID FROM EMPLOYEES E2 WHERE E1.EMPLOYEE_ID = E2.MANAGER_ID;
...
-- 이거 킹 나오면 ?? 매니저인 소수가 뽑혔는데 그게 아닌 애들은 NELL 인 애 뿐인가?
-- 직원아이디가 아니라 매니저 아이디만 넘겨서!!!
SELECT FIRST_NAME, LAST_NAME
FROM EMPLOYEES E1
...
-- 위에위에랑 똑같아.
-- 기본은 직원중에 매니저를 봅아서 매니저가 아닌 애들을 뽑는거야.
SELECT FIRST_NAME, LAST_NAME
FROM EMPLOYEES E1
WHERE NOT EXISTS (SELECT DISTINCT 1
                  FROM EMPLOYEES E2
